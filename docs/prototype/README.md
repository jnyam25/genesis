...
- two **ESP32 station nodes**: #1 runs the label applicator and barcode scanner, #2 runs the robotic capping arm, the lid press and the sort sensor. They reach the PLC over **Wi-Fi** through an access point on the control switch;
...
Station order: labeling → barcode scan → Tank 1/2/3 fill → capping arm → lid press → sort sensor → reject diverter → sort diverter (lane A / lane B).
...
| [esp32.md](esp32.md) | Building the scanner node and the station node |
| [scada.md](scada.md) | Installing and configuring FUXA for history, alarms and trends |
...
└──────┬─────────────────────┬─────────────────────────┬────────────────────────────┘
 ...
 diverters, stack      status)              │ label applicator,    │  │ capping arm, lid press,  │
 light                                      │ barcode → Recipe.*   │  │ sort sensor → Station.*  │
                                            └──────────────────────┘  └──────────────────────────┘
...
| **Micro850 PLC** | Line supervision: container tracking, stoppers or belt stops, valve timing, reject/QC decision, sort lane, counters, OEE, event ring; E-Stop state, reset/start/stop logic, LOCAL/REMOTE authority, the digital E-Stop output; `Station.RunPermit` for the ESP32 nodes | Wait for the monitoring PC. The line (and the local panel) keeps working if the PC or HMI goes away |
| **ESP32 nodes** | Label applicator, barcode parsing, capping arm sequence, lid press, sort sensor, each on a PLC request and only while `Station.RunPermit` = 1 and the PLC heartbeat is alive | Anything safety-related. Their actuators are powered through the safety relay ([safety.md §2a](safety.md#2a-actuators-driven-by-the-esp32-nodes)) |
...
- **The ESP32 nodes run the fiddly stations.** Servo sequences, arm kits and barcode parsing are awkward in ladder logic and cheap on an ESP32. The PLC stays in charge through a request/done handshake per station, so a node can't move a bottle on its own.
- **One register map is the contract.** [`twin/src/plc/tag-map.ts`](../../twin/src/plc/tag-map.ts) defines every address. The virtual PLC, the bridge and the documentation tables are all generated from or checked against it, so the PLC programmer, the firmware developers and the PC side can't drift apart.
...
| 2. Virtual PLC | `npm run dev:plc-sim` (+ FUXA from `deploy/fuxa`) | The **Modbus protocol path**: virtual PLC ↔ bridge ↔ HMI, and virtual PLC ↔ FUXA |
...
- **Stage 2** runs [`twin/src/plc/virtual-plc.ts`](../../twin/src/plc/virtual-plc.ts), which exposes the simulated line on the exact register map the Micro850 must implement. Use it as the **reference implementation** when writing the PLC program. Point ESP32 firmware at it (`<dev PC IP>:5020`) to test nodes before the PLC exists.
- **Automated tests** (`npm test`) cover the Modbus implementation, the register encoding, the station handshake registers and command handshakes against the virtual PLC.

## What has and hasn't been verified

| Item | Status |
| --- | --- |
| Twin engine (BOM station order, 3 E-Stops, sort lanes), HMI, Modbus client/server, virtual PLC ↔ bridge round trip | Automated tests |
| Micro850 program, ESP32 firmware, wiring | **Not written or tested yet.** These documents are the specification for them |
...
| FUXA | Chosen and configuration specified ([scada.md](scada.md)); **not installed or tested yet** (commissioning §3) |
